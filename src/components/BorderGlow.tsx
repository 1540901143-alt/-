...
function parseHSL(hslStr: string): HSL {
  // Supports space separated H S L values like "210 50 50" or similar
  const match = hslStr.match(/([\d.]+)\s+([\d.]+)%?\s+([\d.]+)%?/);
  if (!match) {
    // try fallback with comma separated or single values
    const commaMatch = hslStr.match(/([\d.]+),\s*([\d.]+)%?,\s*([\d.]+)%?/);
    if (commaMatch) {
      return { h: parseFloat(commaMatch[1]), s: parseFloat(commaMatch[2]), l: parseFloat(commaMatch[3]) };
    }
    return { h: 210, s: 80, l: 80 }; // Default blue-ish HSL
  }
...
function buildGlowVars(glowColor: string, intensity: number): Record<string, string> {
  ...
  for (let i = 0; i < opacities.length; i++) {
    const key = `--glow-color${keys[i]}`;
    const value = `hsl(${base} / ${Math.min(opacities[i] * intensity, 100)}%)`;
    vars[key] = value;
  }
  return vars;
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  className = '',
  edgeSensitivity = 30,
  glowColor = '210 80 80', // HSL components
  backgroundColor = '#FCFAF7', // Updated for light theme match
  borderRadius = 28,
  glowRadius = 40,
  glowIntensity = 1.0,
  coneSpread = 25,
